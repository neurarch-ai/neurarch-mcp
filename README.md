...
0.14 remeasured after the parser learned to follow same-file classes, container loops and multi-line constructors ([`docs/REAL_REPOS_STUDY_2.md`](./docs/REAL_REPOS_STUDY_2.md), same 116 files): a graph for 86% of files, a recognisable one for 41% (47 of 116, up from 9), median 8 layers. Two caveats travel with that number: Hugging Face `modeling_*.py` files now parse to 46 to 135 nodes but every `config.hidden_size` is still text, so their counts are a guess and their shape rules stay suppressed; and the second study's findings were not hand-judged, so the zero-real-bugs result of the first one stands as the only precision measurement.
